...
void Graphics::Render()
{
    ...
    try
    {
        ...
        if (acquireResult.result == vk::Result::eSuboptimalKHR)
        {
            ...
            // the acquire signalled frameImageReady; returning now would leave it signalled with no
            // matching wait, so render this frame anyway and rebuild the swapchain on the next one
